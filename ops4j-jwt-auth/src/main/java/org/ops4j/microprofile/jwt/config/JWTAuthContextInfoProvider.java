 ...
 *
 */
package org.ops4j.microprofile.jwt.config;
...
public class JWTAuthContextInfoProvider {
    ...
    private static final Logger log = LoggerFactory.getLogger(JWTAuthContextInfoProvider.class);
    ...
    Optional<JWTAuthContextInfo> getOptionalContextInfo() {
        ...
        log.debug("init, mpJwtublicKey={}, mpJwtIssuer={}, mpJwtLocation={}",
        ...
        if (mpJwtublicKey.isPresent() && !NONE.equals(mpJwtublicKey.get())) {
            ...
            try {
                ...
                log.debug("mpJwtublicKey parsed as JWK(S)");
            } catch (Exception e) {
                // Try as PEM key value
                log.debug("mpJwtublicKey failed as JWK(S), {}", e.getMessage());
                try {
                    RSAPublicKey pk = (RSAPublicKey) KeyUtils.decodePublicKey(mpJwtublicKey.get());
                    contextInfo.setSignerKey(pk);
                    log.debug("mpJwtublicKey parsed as PEM");
                } catch (Exception e1) {

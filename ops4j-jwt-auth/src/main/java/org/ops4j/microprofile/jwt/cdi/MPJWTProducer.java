...
public class MPJWTProducer {
    private static Logger log = LoggerFactory.getLogger(MPJWTProducer.class);
    ...
    void observeRequestInitialized(@Observes @Initialized(RequestScoped.class) Object event) {
        log.trace("observeRequestInitialized, event=%s", event);
    }

    void observeRequestDestroyed(@Observes @Destroyed(RequestScoped.class) Object event) {
        log.trace("observeRequestDestroyed, event=%s", event);
    }
    ...
    public static <T> T getValue(String name, boolean isOptional) {
        ...
        if (jwt == null) {
            log.debug("getValue(%s), null JsonWebToken", name);
            return null;
        }

        Optional<T> claimValue = jwt.claim(name);
        if (!isOptional && !claimValue.isPresent()) {
            log.debug("Failed to find Claim for: %s", name);
        }
        log.debug("getValue(%s), isOptional=%s, claimValue=%s", name, isOptional, claimValue);
